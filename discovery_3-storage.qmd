# Lotterhos Lab vs. Personal Storage on Explorer; Syncing to Github

------------------------------------------------------------------------

-   We have **active** storage (35 TB) that should be used for 'active' projects, and **archival** storage for storing raw files and outputs from projects. Please be smart about how you use archival storage and work with Dr. Lotterhos to make sure only relevant files are being archived.

-   In addition each user has a home directory and a scratch directory.

## Storage unix tricks

`df -h /projects/lotterhos` #total space useage for lotterhos. We have 36 TB of available storage and this will show you what is available.

`pwd | du -h --max-depth=1` get disk use for folders in the current directory. If you want to sort the output by size, add the pipe `| sort -rh`. This can take a while if folders have several TB, so if that is the case do this:
- You can also run a bash script to estimate disk usage stored in `/projects/lotterhos/0diskUsage.sh` by running `sbatch 0diskUsage.sh`. After this runs, make sure to add the date to the `.out` file for comparison
...
Then in the command line in Explorer use your token when it asks for a password.

It is good practice to create a repo on Github and then clone it to Explorer to start a project. You can follow instructions in Buffalo's Bioinformatics book to work with git on linux.

In addition, you may want to collaborate on Explorer through your git repo.

You may also need to set up ssh keys:

example: When person A started working on his git branch in Explorer, he cloned that branch to Explorer (e.g in the folder and repo /work/lotterhos/MVP-NonClinalAF). To give person B access,
    ...
    6. Click Add SSH key.
    7. Test the connection by running the following command in the terminal on person B Explorer:
...
### **Your home directory on Explorer\*\*.** {.unnumbered}

-   Path: `/~/` or `/home/username`

-   Note that only you have access to these files. Store things here that are not relevant to lab research.

### **Your scratch directory on Explorer\*\*. Use this for intermediate outputs that are not backed up.** {.unnumbered}

-   Path: `/scratch/username`

-   Note that only you have access to these files.
- This is where you should store outputs for workflows that generate a large number of files. Please only save the files you need in the /projects/lotterhos

### **Lotterhos shared storage on Explorer** {.unnumbered}
...
- We have 25 TB here, about 5-10 are in use as of summer 2026.
- Make sure before transferring a folder here that you tarball it:
`tar -czvf directoryName.tar.gz /path/to/directoryName`

In the folder there is a file called `collection-space-usage.txt`. You can click on "Open" and view the file in another window. 

![](img/storage-NESE.png)
![](img/storage-NESE-space.png)

Use Globus to do the transfer (see the Globus page)

# Outdated storage [ignore from here down; maintain for historical purposes]
...
-   From a laptop or over wifi, transfers of large files are very slow. This is why we map to the server through the iMac. The point of all the steps we outline below is so we can use Globus to transfer files from the smb drive to Explorer.
