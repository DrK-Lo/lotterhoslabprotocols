...
In the following command, the `displayname` is what is shown in the figure above. The `name` argument should be similar to the display name; `name` is used in the background for naming filesystems. Since you may have multiple R environments with a specific 
