...
muts_maf_thinned_filtered <- muts_maf_filtered[thinned_snps, ]
snp_names <- muts_maf_thinned_filtered$Affx.ID
colnames(gen_filtered_subset) <- snp_names
ind_names <- samp_full_subset$merge_ID_inds
rownames(gen_filtered_subset) <- ind_names
