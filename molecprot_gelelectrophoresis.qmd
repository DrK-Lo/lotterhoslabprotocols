...
1.	Mix the gel components.For a 1% agarose gel: 
         i.	Small gel: 0.60g agarose + 60 ml 1xTAE Buffer
    b.	Add agarose, then 1xTAE to an Erlenmeyer flask.
    c.	Use the magnetic stirring plate (put a magnet inside the Erlenmeyer flask) to mix. Turn the RPM into the 1000s until all the clumps are gone. 
2.	Melt the gel.
...
4.	Add GelRed:
    ...
    b.	Gently swirl until the stain is dissolved (you should not be able to see any red stain in the gel).
5.	Pour the gel.
    a.	Use masking tape to create walls on either side of the tray to contain the solution when poured. Use multiple layers of tape to make sure it doesn’t leak.
...
6.	Let the gel set for ~40 minutes.
...
7.	Make sure that the power source is off, and the gel rig is unplugged before removing the top.
8.	Place the tray with your gel in the gel rig with wells on the side with the black plug 
    a.	Samples will run from the black side toward red side.
    b.	DNA “runs to red”
9.	Fill rig with enough 1xTAE Buffer to just cover the entire gel.
...
10.	Load the gel.
    ...
    i.	W use 3ul DNA + 1ul loading dye.
...
12.	Run the gel.
    ...
    b.	Check for bubbles rising at cathode (black plug) side to ensure the rig in working. If you do not see bubbles rising, the gel is not running.
    c.	When the gel is done running, turn the off power. 
        i.	Never open the rig when the power is on.
13.	Visualize the gel.
...
14.	Always clean the gel doc tray after use.
15.	Dispose of the gel in proper mayo jar. 
16.	Rinse gel tray and let dry on lab bench.
...
The voltage machine in the gel room will often give an "E1" error and will not begin running. When this happens, check that there is enough TAE buffer in the rig such that the electrodes are touching the liquid. Add a bit more if not. Also check that electrodes are tightened on each side. 
